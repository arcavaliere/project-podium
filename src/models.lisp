...
(defun change-project (id &key name leader summary status remove-leg leg add-member remove-member member)
  (let ((project (mito:find-dao 'project :id id)))
    (cond ((not (equal name nil)) (setf (slot-value project 'project-name) name))
          ((not (equal leader nil)) (setf (slot-value project 'project-leader) leader))
          ((not (equal summary nil)) (setf (slot-value project 'project-summary) summary))
          ((not (equal status nil)) (setf (slot-value project 'project-status) status))
          ((and (equal remove-leg t) (not (eq leg nil))) (remove-leg-from-project-and-db (mito:object-id leg)))
          ((and (equal add-member t) (not (eq member nil))) (add-member-to-project project member))
          ((and (equal remove-member t) (not (eq member nil))) (remove-member-from-project project member)))
...
(defun change-project-leg (id &key project leader deliverable due-date status add-member remove-member member)
  (let ((leg (mito:find-dao 'project-leg :id id)))
    (cond ((not (equal project nil)) (setf (slot-value leg 'project-leg-project) project))
          ((not (equal leader nil)) (setf (slot-value leg 'project-leg-leader-id) leader))
          ((not (equal deliverable nil)) (setf (slot-value leg 'project-leg-deliverable) deliverable))
          ((not (equal due-date nil)) (setf (slot-value leg 'project-leg-due-date) due-date))
          ((not (equal status nil)) (setf (slot-value leg 'project-leg-status) status))
          ((and (equal add-member t) (not (eq member nil))) (add-member-to-leg leg member))
          ((and (equal remove-member t) (not (eq member nil))) (remove-member-from-leg leg member)))
    (mito:save-dao leg)
    leg))
